...
const PERSON_COLOR_OVERRIDES = { 'Miranda Osborn': '#9467bd' };   // purple (was orange)
const _personColors = Object.assign({}, PERSON_COLOR_OVERRIDES);
let _personColorN = 0;
function personColor(name){
  if (name in _personColors) return _personColors[name];
  // Skip palette entries already taken (by an override or an earlier person) to avoid clashes.
...
async function renderDashboard() {
  ...
  function renderEstProj() {
    // Stacked bar chart of estimated hours per project, split by assignee. Project list/caps
    // come from estData; the per-person split is inverted out of teamData.breakdown.
    if (!estData || !teamData) { view.innerHTML = '<p class="muted">Loading widgets…</p>'; return; }
    // Invert the team breakdown (person → projects) into project name → { person: est hours }.
    const byProject = {};
    Object.entries(teamData.breakdown).forEach(([person, projs]) => (projs || []).forEach(p => {
      if (!p.estimated) return;
      (byProject[p.project] = byProject[p.project] || {});
      byProject[p.project][person] = (byProject[p.project][person] || 0) + p.estimated;
    }));
    const rows = estData.filter(w => w.hours > 0).slice().sort((a, b) => b.hours - a.hours);
    if (!rows.length) { view.innerHTML = '<p class="muted">No estimated hours.</p>'; return; }
    view.innerHTML = '<div class="chart-box"><canvas id="chart"></canvas></div>' +
      '<div id="est-summary"></div>';
    if (chart) { chart.destroy(); chart = null; }
    const labels = rows.map(w => w.name);
    const gids = rows.map(w => w.gid), ntasks = rows.map(w => w.ntasks);
    const caps = rows.map(w => (w.cap == null ? null : w.cap));
    const datasets = personStacks(rows, w => byProject[w.name] || {});
    ...
    chart = new Chart(document.getElementById('chart'), {
      ...
      options: { responsive: true, maintainAspectRatio: false,
        ...
        onClick: (evt, els) => { if (els.length) { const g = gids[els[0].index]; if (g) location.hash = '#/p/' + g; } },
        ...
        plugins: { legend: { display: true, position: 'bottom' }, capMarks: { caps },
          tooltip: { itemSort: (a, b) => b.parsed.y - a.parsed.y, filter: item => item.parsed.y > 0,
            callbacks: {
              ...
              afterBody: items => {
                const i = items[0].dataIndex, lines = [`Total ${h2(rows[i].hours)} h · ${ntasks[i]} tasks`];
                if (caps[i] != null) lines.push(`Budget ${h2(caps[i])} h/mo (${(rows[i].hours / caps[i] * 100).toFixed(0)}%)`);
                return lines;
              } } } } }
    });
    renderEstSummary(rows);
  }
        ...
        plugins: { legend: { display: true, position: 'bottom' }, capMarks: { caps },
          tooltip: { itemSort: (a, b) => b.parsed.y - a.parsed.y, filter: item => item.parsed.y > 0,
            callbacks: {
              label: ctx => `${ctx.dataset.label}: ${h2(ctx.parsed.y)} h`,
              afterBody: items => {
                const i = items[0].dataIndex, lines = [`Total ${h2(rows[i].hours)} h · ${ents[i]} entries`];
                if (caps[i] != null) lines.push(`Budget ${h2(caps[i])} h billable`);
                return lines;
              } } } } }
    });
  }
