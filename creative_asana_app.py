...
async function renderDashboard() {
  ...
  function renderActualItems() {
    ...
    const itemRow = (it) =>
      `<tr><td class="muted">${esc(it.project)}</td><td>${esc(it.task)}</td><td>${esc(it.person)}</td>` +
    ...
    view.innerHTML = picker +
      `<p class="drill-total">${items.length} item(s) · ${totE} time entr${totE === 1 ? 'y' : 'ies'} · ` +
       ...
       <table class="tasks">
         <thead><tr><th>Project</th><th>Item</th><th>Logged by</th><th class="hours">Billable</th><th class="hours">Unbillable</th><th class="hours">Total</th></tr></thead>
         <tbody>${items.map(itemRow).join('')}
           <tr class="parent"><td>All projects</td><td></td><td></td>` +
