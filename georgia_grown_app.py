...
const TABS = {
  ...
  actualproj: { label:'Bar Chart', title:'Bar Chart',
    sub:'Billable vs. unbillable hours logged per project in the selected date range — only billable counts toward budget. Click a bar to drill into that project.' },
  ...
  june: { label:'Statistics', title:'Statistics',
    sub:'Time-tracking entries logged in the selected date range, split billable / unbillable. Click a widget to drill in.' },
  capacity: { label:'MSA Project Capacity', title:'MSA Project Capacity',
    sub:'Projects with a monthly hour budget — selected range’s billable usage vs. capacity (unbillable excluded). Click to drill in.' },
};
// Sidebar groups: estimated/planned views vs. logged-hours & progress views.
const NAV_SECTIONS = [
  { title: 'Estimated Hours', tabs: ['team', 'estimated'] },
  { title: 'Actual Hours', tabs: ['actualproj', 'june', 'capacity'] },
];
...
async function renderDashboard() {
  ...
  function renderActualProj() {
    ...
    // Roll grouped projects (e.g. CMD) into one combined bucket with the group's cap;
    // every other project stays on its own bar. Then drop anything with no hours.
    ...
    const groupItems = (groupsConfig || []).map(g => {
      ...
      return { name: g.name, gid: null, isGroup: true, cap: g.cap, members: s.members,
    ...
    const rows = [...groupItems, ...projItems]
      .filter(w => w.hours > 0)
    ...
    view.innerHTML = picker + '<div class="chart-box"><canvas id="chart"></canvas></div>' +
      `<p class="hint">Billable vs. unbillable hours logged per project, ${rangeLabel(dateStart, dateEnd)}. CMD is one combined bucket — hover it to see each sub-project. Only billable counts toward the monthly budget (amber line); the billable bar turns red when over budget. Click a bar to drill in.</p>` +
      '<div id="actual-summary"></div>';
    wireRangeSel();
    if (chart) { chart.destroy(); chart = null; }
    const labels = rows.map(w => w.name);
    const billData = rows.map(w => w.billable_hours);
    const unbillData = rows.map(w => w.unbillable_hours);
    const gids = rows.map(w => w.gid), ents = rows.map(w => w.nentries);
    const isGrp = rows.map(w => !!w.isGroup);
    const caps = rows.map(w => (w.cap == null ? null : w.cap));
    const members = rows.map(w => w.members || null);   // member breakdown for combined buckets
    // Billable bar turns red only when *billable* hours blow the budget.
    const billColors = rows.map(w => (w.cap != null && w.billable_hours > w.cap) ? '#e26b66' : '#4cc085');
    const billBorders = rows.map(w => (w.cap != null && w.billable_hours > w.cap) ? '#e98e8a' : '#6cd49d');
    // Keep every budget marker visible even if it sits above the tallest bar.
    const top = Math.max(...billData, ...unbillData, ...caps.filter(c => c != null), 1);
    chart = new Chart(document.getElementById('chart'), {
      type: 'bar',
      data: { labels, datasets: [
        { label: 'Billable', data: billData, backgroundColor: billColors, borderColor: billBorders, borderWidth: 1 },
        { label: 'Unbillable', data: unbillData, backgroundColor: '#8a929c', borderColor: '#a3aab4', borderWidth: 1 } ] },
      options: { responsive: true, maintainAspectRatio: false,
        // Single projects drill into their detail; combined buckets (gid null) don't navigate.
        onClick: (evt, els) => { if (els.length) { const g = gids[els[0].index]; if (g) location.hash = '#/june/' + g; } },
        onHover: (evt, els) => { evt.native.target.style.cursor = (els.length && gids[els[0].index]) ? 'pointer' : 'default'; },
        ...
        plugins: { legend: { display: true }, capMarks: { caps },
          tooltip: { callbacks: {
            label: ctx => `${ctx.dataset.label}: ${h2(ctx.parsed.y)} h`,
            afterBody: items => {
              const i = items[0].dataIndex, lines = [`Entries: ${ents[i]}`];
              if (caps[i] != null) lines.push(`Budget ${h2(caps[i])} h billable (${(billData[i] / caps[i] * 100).toFixed(0)}%)`);
              const mem = members[i];
              if (mem && mem.length) {
                lines.push('— by sub-project (billable) —');
                mem.slice().sort((a, b) => b.billable_hours - a.billable_hours)
                   .forEach(m => lines.push(`${m.name}: ${h2(m.billable_hours)} h`));
              }
              return lines;
            } } } } }
    });
  ...
  function renderActualSummary() {
    ...
    // By project — straight from the loaded range summaries (individual projects).
    const projRows = juneData.filter(w => w.hours > 0).slice().sort((a, b) => b.hours - a.hours);
  ...
  // Pull each logged project's detail and sum billable/unbillable per person, then cache + re-render.
  async function loadPersonStats(key) {
    ...
    const agg = {};
    details.forEach(d => (d.labels || []).forEach((name, i) => {
    ...
    if (dashTab === 'actualproj') renderActualSummary();
  }
  ...
  function setUpdatedLabel() {
    ...
    if (!el) return;
    let u = null;
    if (dashTab === 'team') u = teamData && teamData.updated;
    else if (dashTab === 'estimated') u = estData && estData[0] && estData[0].updated;
  ...
  function renderTab() {
    ...
    if (dashTab === 'team') {
      ...
      if (!juneData) { view.innerHTML = picker + loading; wireRangeSel(); }
        ...
        // Combined budget buckets first, then any standalone capped project that
        // isn't part of a group (group members are rolled into their bucket).
        const memberGids = new Set((groupsConfig || []).flatMap(g => g.gids));
        ...
        if (!groups.length && !standalone.length) {
          view.insertAdjacentHTML('beforeend', '<p class="muted">No projects have a monthly capacity set.</p>');
        } else {
          const grid = document.createElement('div');
          grid.className = 'grid';
          groups.forEach(g => grid.appendChild(groupCard(g)));
          standalone.forEach(w => grid.appendChild(capCard(w)));
          view.appendChild(grid);
        }
        wireRangeSel();
      }
  ...
  async function loadJune(refresh) {
    ...
    personStatsCache = {};   // range/refresh changed — recompute per-person totals
    try {
